...
# Output
OUT_ROOT    = ROOT.with_name(ROOT.name + "_denadapt")
...
for ann_path in tqdm(ann_paths,
    ...
    img_resized = resize_img(img, RESIZE_WIDTH)
    ...
    cls_scales = [ [] for c in ISAID_CLASSES ]
    ...
    with ann_path.open("r") as f:
        for line in f:
            ...
            cls_scales[cls_id].append( ((w + h) / 2) * (RESIZE_WIDTH / orig_w) )
            
            cx = int(x + w / 2.0)
            cy = int(y + h / 2.0)
            cx, cy = resize_coord((cx, cy), (orig_h, orig_w), RESIZE_WIDTH)
            cx = np.clip(cx, 0, resized_w - 1)
            cy = np.clip(cy, 0, resized_h - 1)
            kpoint[cls_id, cy, cx] = 1


    # Take the average size for the gausian
    scales = [ (float(KERNEL_SIZE[i]) if len(s) == 0 else float(np.mean(s))) for i, s in enumerate(cls_scales) ]
    ...
    for cls in range(len(ISAID_CLASSES)):
        density_map[cls] = gaussian_filter(
            kpoint[cls].astype(np.float32), sigma=scales[cls]
        )
